...
{{-- NOTIFIKASI TOAST MELAYANG DENGAN ANIMASI FADE & SLIDE (Halus & Elegan) --}}
...
<div class="row g-3" id="gridKategori">
  ...
  <div class="col-sm-6 col-lg-4">
    <div class="rounded-3 p-4 text-white text-center position-relative kategori-card"
         data-id="{{ $k->id }}"
         data-nama="{{ $k->nama }}"
         data-icon="{{ $k->icon ?? 'bi bi-box' }}"
         data-warna="{{ $k->warna }}"
         style="background:{{ $k->warna }};cursor:pointer;min-height:120px;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:.5rem;"
      ...
      <div class="position-absolute" style="top: 10px; right: 12px; display: flex; gap: 0.35rem;">
        ...
        <button type="button"
                ...
                onclick="event.stopPropagation(); konfirmasiHapusKategori({{ $k->id }}, '{{ addslashes($k->nama) }}', this)"
